...
public:
    ...
    bool Execute(DataSyncScanCc &req) override
    {
             ...
             ++scan_cnt)
        {
            ...
            {
                // `cce->NeedCkpt() && cce->CommitTs() <= SchemaTs()` means the
                // secondary index is in dirty state.
                // If the sk ccmap is in dirty state, the sk generated by pk may
                // be inserted to ccmap between the two requests of
                // `ScanSliceDeltaSizeCc` and `DataSyncScanCc`. The data store
                // size of these cces are not updated and are still in unknown
                // state
                if (slice_pinned ||
                    ((table_name_.Type() == TableType::Secondary ||
                      table_name_.Type() == TableType::UniqueSecondary) &&
                     cce->NeedCkpt() && cce->CommitTs() <= SchemaTs() &&
                     !req.StoreRangePtr()->HasDmlSinceDdl()))
                {
                    ...
                    cce->entry_info_.SetDataStoreSize(0);
                         ...
                         req.StoreRangePtr()->HasDmlSinceDdl())
                {
                    // The secondary index is in dirty state, and there are
                    // concurrent dml transaction during the ddl transaction,
                    // should load data store size by pinning the slice. Data
                    // store size is required to decide slice & range update
                    // plan.
                    RangeSliceOpStatus pin_status;
                    RangeSliceId slice_id = shard_->local_shards_.PinRangeSlice(
                        table_name_,
