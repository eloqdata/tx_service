...
class SequenceKeySchema : public txservice::KeySchema
{
public:
    explicit SequenceKeySchema(uint64_t key_schema_ts)
        : schema_ts_(key_schema_ts)
    {
    }

    using Uptr = std::unique_ptr<SequenceKeySchema>;
...
struct SequenceRecordSchema : public txservice::RecordSchema
{
public:
...
{
public:
    SequenceTableSchema(const txservice::TableName &sequence_table_name,
                        const std::string &catalog_image,
                        uint64_t version);

    txservice::TableSchema::uptr Clone() const override
    {
        return std::make_unique<SequenceTableSchema>(
            sequence_table_name_, schema_image_, version_);
    }

    const txservice::TableName &GetBaseTableName() const override
    {
        return sequence_table_name_;
    }
...
private:
    txservice::TableName sequence_table_name_;
    txservice::KVCatalogInfo::uptr kv_info_{nullptr};
    SequenceKeySchema::Uptr key_schema_{nullptr};
    SequenceRecordSchema::Uptr record_schema_{nullptr};
