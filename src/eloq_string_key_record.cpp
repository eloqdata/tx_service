...
void EloqStringRecord::Serialize(std::vector<char> &buf, size_t &offset) const
{
    ...
    std::copy(blob_size_ptr, blob_size_ptr + sizeof(uint32_t), buf.begin() + offset);
...
{
    const EloqStringRecord &typed_rhs = static_cast<const EloqStringRecord &>(rhs);
...
TxRecord::Uptr EloqStringRecord::AddTTL(uint64_t ttl)
{
    ...
    throw std::runtime_error("EloqStringRecord does not support TTL conversion");
}
