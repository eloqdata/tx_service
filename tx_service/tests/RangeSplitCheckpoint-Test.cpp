...
TEST_CASE("split-range checkpoint updates source CCE shard", "[checkpoint]")
{
    ...
    TestKey child_start = std::make_tuple(std::string("split_checkpoint"), 100);
    ...
    TestKey record_key =
        std::make_tuple(std::string("split_checkpoint"), record_key_suffix);
    ...
    TableName hash_table_name(std::string("split_checkpoint_hash"),
                              TableType::Primary,
                              TableEngine::EloqKv);
    DataSyncTask hash_task(
        hash_table_name, 3, 0, 0, 1, 0, nullptr, false, false, nullptr);
