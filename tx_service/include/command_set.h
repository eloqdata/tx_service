...
public:
    ...
    {
        CmdSetEntry &entry = UpsertObjectCmdEntry(table_name,
                                                  cce_addr,
                                                  payload_status,
                                                  cce_version,
                                                  lock_ts,
        ...
        if (cmd != nullptr)
        {
            ...
            SetupForwardEntry(entry, key, forward_key_shard);
        }
    }
...
private:
    // Looks up (or creates) the per-cce entry and refreshes the fields a txn
    // updates on every command against the same cce. Shared by
    // AddObjectCommand and AddObjectCommandImage so the bookkeeping cannot
    // drift between the two.
    CmdSetEntry &UpsertObjectCmdEntry(const TableName &table_name,
                                      const CcEntryAddr &cce_addr,
                                      RecordStatus payload_status,
                                      uint64_t cce_version,
                                      uint64_t lock_ts,
