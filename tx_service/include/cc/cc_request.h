...
public:
    ...
    bool AllowRunOnCandidate() const
    {
        return allow_run_on_candidate_;
    }
...
private:
    ...
    // True if this ccrequest is allowed to run on candidate node, such as
    // issued in a recovering process
    bool allow_run_on_candidate_{false};
...
public:
    ...
    bool ValidTermCheck()
    {
        ...
        int64_t current_term =
            std::max({cc_ng_term, standby_node_term, candidate_ng_term});
    ...
    int64_t GetNodeGroupLeaderTerm() const
    {
        int64_t leader_term = Sharder::Instance().LeaderTerm(node_group_id_);
        int64_t candidate_term =
            Sharder::Instance().CandidateLeaderTerm(node_group_id_);
        return std::max(leader_term, candidate_term);
    }
...
public:
               ...
               const bool is_lock_recovery = false)
    {
        ...
        log_records_sv_ =
            std::string_view(replay_message_->binary_log_records().data(),
                             replay_message_->binary_log_records().size());
        cc_ng_id_ = cc_ng_id;
        cc_ng_term_ = cc_ng_term;
        assert(cc_ng_term_ >= 0);
        mux_ = &mux;
    ...
    bool Execute(CcShard &ccs) override
    {
        ...
        while (offset < log_records_sv_.size())
        {
            ...
            std::string_view blob(log_records_sv_.data() + offset, blob_length);
            offset += blob_length;
...
private:
    std::unique_ptr<::txlog::ReplayMessage> replay_message_{nullptr};
    std::string_view log_records_sv_;
    uint32_t cc_ng_id_;
    int64_t cc_ng_term_{-1};
    bthread::Mutex *mux_;
