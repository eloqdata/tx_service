...
public:
    ...
    bool Execute(ReadCc &req) override
    {
        ...
        bool bucket_lock_acquired = false;

        // This is a resumed request from blocking queue.
        if (req.CcePtr() != nullptr)
        {
            if (req.CcePtr() != floor_cce)
            {
                // The request is resumed from blocking queue, and the
                // previously blocked range cc entry is different from the
                // currently found range cc entry. So we need to release the
                // previous range cc entry and its bucket record locks.
                req.SetBlockType(ReadCc::NotBlocked);
                CcEntry<KeyT, RangeRecord, true, false> *prev_cce =
                    static_cast<CcEntry<KeyT, RangeRecord, true, false> *>(
                        req.CcePtr());
                auto prev_bucket_cce = static_cast<
                    CcEntry<RangeBucketKey, RangeBucketRecord, true, false> *>(
                    prev_cce->payload_.cur_payload_->range_owner_rec_);
                // Release previous bucket record read lock
                ReleaseCceLock(prev_bucket_cce->GetKeyLock(),
                               prev_bucket_cce,
                               req.Txn(),
                               req.NodeGroupId(),
                               LockType::ReadLock);
                // Release previous range record read lock
                ReleaseCceLock(prev_cce->GetKeyLock(),
                               prev_cce,
                               req.Txn(),
                               req.NodeGroupId(),
                               LockType::ReadLock);
        ...
        else if (req.CommitType() == PostWriteType::PostCommit)
        {
            ...
            for (uint idx = 0; idx < new_range_entries.size(); idx++)
            {
                ...
                if (new_range_owner == this->cc_ng_id_ &&
                    static_cast<uint16_t>(new_range_id % shard_->core_cnt_) ==
                        shard_->core_id_)
                {
            ...
            int32_t old_partition_id =
                upload_range_rec->GetRangeInfo()->PartitionId();
            if (range_owner == this->cc_ng_id_ &&
                static_cast<uint16_t>(old_partition_id % shard_->core_cnt_) ==
                    shard_->core_id_)
            {
                TableType data_table_type =
                    TableName::Type(this->table_name_.StringView());
            ...
            !is_coordinator)
        {
            ...
            NodeGroupId range_owner =
                shard_->GetRangeOwner(old_range_id, this->cc_ng_id_)
                    ->BucketOwner();
            if (range_owner == this->cc_ng_id_ &&
                static_cast<uint16_t>(old_range_id % shard_->core_cnt_) ==
                    shard_->core_id_)
            {
                size_t old_range_size =
