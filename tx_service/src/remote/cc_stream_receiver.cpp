...
void CcStreamReceiver::OnReceiveCcMsg(std::unique_ptr<CcMessage> msg)
{
    ...
    case CcMessage::MessageType::CcMessage_MessageType_ScanSliceRequest:
    {
        ...
        uint16_t dest_core = (range_id & 0x3FF) % local_shards_.Count();
        local_shards_.EnqueueCcRequest(dest_core, scan_slice_req);
        ...
        CcMessage_MessageType_KeyObjectStandbyForwardRequest:
    {
        ...
        LOG(INFO) << "Recv ForwardReq, keystr: " << req.key()
                  ...
                  << ", seq group: " << req.forward_seq_grp()
                  << ", seq id: " << req.forward_seq_id()
                  << ", out of sync: " << req.out_of_sync();
