...
BatchReadCatalogOperation::BatchReadCatalogOperation(TransactionExecution *txm)
{
    CcHandlerResult<ReadKeyResult> &hd_res = hd_result_vec_.emplace_back(txm);
    hd_res.post_lambda_ = [this](CcHandlerResult<ReadKeyResult> *res)
    {
...
void BatchReadCatalogOperation::Reset()
{
    const std::vector<std::string> *table_names = batch_req_->table_names_;
    const size_t N = table_names->size();

    catalog_keys_.clear();
    catalog_records_.clear();
    catalog_keys_.reserve(N);
    catalog_records_.reserve(N);
    for (size_t i = 0; i < N; ++i)
    {
        catalog_keys_.emplace_back(TableName(std::string_view((*table_names)[i]),
                                            TableType::RangePartition,
                                            TableEngine::None));
        catalog_records_.emplace_back();
    }

    if (N > hd_result_vec_.size())
    {
        hd_result_vec_.reserve(N);
    }
    else
    {
        while (hd_result_vec_.size() > N)
        {
            hd_result_vec_.pop_back();
        }
    }
    for (size_t i = 0; i < N; ++i)
    {
        if (i < hd_result_vec_.size())
        {
            hd_result_vec_[i].Value().Reset();
            hd_result_vec_[i].Reset();
        }
        else
        {
            CcHandlerResult<ReadKeyResult> &head = hd_result_vec_[0];
            CcHandlerResult<ReadKeyResult> &new_res =
                hd_result_vec_.emplace_back(head.Txm());
            new_res.post_lambda_ = head.post_lambda_;
        }
    }

    unfinished_cnt_.store(static_cast<uint32_t>(N), std::memory_order_relaxed);
}

void BatchReadCatalogOperation::Forward(TransactionExecution *txm)
{
    if (!IsFinished())
    {
        for (size_t i = 0; i < hd_result_vec_.size(); ++i)
        {
            CcHandlerResult<ReadKeyResult> &hd_res = hd_result_vec_[i];
            if (hd_res.IsFinished())
            {
                continue;
            }
            if (!hd_res.Value().is_local_)
            {
                const CcEntryAddr &cce_addr = hd_res.Value().cce_addr_;
                if (cce_addr.Term() > 0 && txm->CheckLeaderTerm())
                {
                    DeadLockCheck::RequestCheck();
                    txm->cc_handler_->BlockCcReqCheck(
    ...
    std::vector<std::pair<bool, CatalogRecord>> *result_out =
        batch_req_->result_out_;
    CcErrorCode first_err = CcErrorCode::NO_ERROR;
    const size_t N = catalog_records_.size();
    ...
    for (size_t i = 0; i < N; ++i)
    {
        ...
        if (hd_res.IsError())
        {
            if (first_err == CcErrorCode::NO_ERROR)
            {
                first_err = hd_res.ErrorCode();
            }
        }
        else if (hd_res.Value().rec_status_ == RecordStatus::Normal)
        {
            const ReadKeyResult &read_res = hd_res.Value();
            txm->rw_set_.AddRead(read_res.cce_addr_,
                                read_res.ts_,
                                &catalog_ccm_name);
            result_out->emplace_back(true, std::move(catalog_records_[i]));
        }
        else
        {
            result_out->emplace_back(false, CatalogRecord{});
        }
    }

    if (first_err != CcErrorCode::NO_ERROR)
    {
        batch_req_->SetError(txm->ConvertCcError(first_err));
    }
    else
    {
        txm->void_resp_->Finish(Void{});
    }
    txm->state_stack_.pop_back();
}
