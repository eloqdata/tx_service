...
{
    LOG(INFO) << "OnStartFollowing, node_id: " << node_id << ", term: " << term
                                  ...
                                  bool resubscribe)
{
    ...
    {
        store_hd->SetStandbySnapshotPayload(ng_id_,
                                            start_follow_resp.snapshot_path());
        DLOG(INFO)
            << "SubscribePrimaryNode set standby snapshot payload, ng_id="
    ...
    if (!txservice_skip_kv)
    {
        ...
        DLOG(INFO) << "CcNode::SubscribePrimaryNode send "
                   ...
                   << ", standby_node_term=" << snapshot_req.standby_node_term()
                   << ", dest_path=" << dest_path;
        cntl.Reset();
