...
DataStoreService::~DataStoreService()
{
    ...
    // Stop prewarm operation (stops timer thread and destroys data store)
    // Note: shard_id is not available in destructor, but this is fine
    // as we're shutting down everything anyway
    // First, stop the standby sync file cache operation
    if (prewarm_data_store_ != nullptr)
    {
        prewarm_data_store_->StopStandbySyncFileCache();
    }

    // Then, stop timer thread if running
    if (prewarm_timer_thread_.joinable())
    {
        prewarm_timer_stop_ = true;
        prewarm_timer_thread_.join();
    }

    // Finally, destroy prewarm data store
    if (prewarm_data_store_ != nullptr)
    {
        DestroyPrewarmDataStore();
    }
#endif
}
...
#if defined(DATA_STORE_TYPE_ELOQDSS_ELOQSTORE)
bool DataStoreService::StartPrewarmOperation(
    uint32_t shard_id, std::unordered_set<uint16_t> &&bucket_ids, int64_t term)
{
    // Check if cloud storage mode is enabled
    if (data_store_factory_ == nullptr ||
        !data_store_factory_->IsCloudStorageMode())
    {
        LOG(INFO)
            << "Not in cloud storage mode, skip starting prewarm operation";
        return false;
    }

    // Check if current node is a standby node for this shard
    std::vector<uint32_t> shards_for_this_node =
        cluster_manager_.GetShardsForThisNode();
    bool is_standby_for_shard =
        std::find(shards_for_this_node.begin(),
                  shards_for_this_node.end(),
                  shard_id) != shards_for_this_node.end();

    if (!is_standby_for_shard)
    {
        LOG(INFO) << "Current node is not a standby node for shard " << shard_id
                  << ", skip starting prewarm operation";
        return false;
    }

    // Check if prewarm data store already exists
    if (prewarm_data_store_ != nullptr)
    {
        LOG(INFO) << "Prewarm data store already exists, stop older and "
                     "restart new one";
        StopPrewarmOperation(shard_id);
    }

    // Create and start prewarm data store
    if (!CreatePrewarmDataStore(shard_id, std::move(bucket_ids), term))
    {
        LOG(ERROR) << "Failed to create prewarm data store";
        return false;
    }

    // Start periodic prewarm timer worker thread
    if (!prewarm_timer_thread_.joinable())
    {
        // Reset stop flag
        prewarm_timer_stop_ = false;

        // Get interval from cluster manager configuration
        uint32_t interval_sec = cluster_manager_.GetFileCacheSyncIntervalSec();

        // Start the timer thread
        prewarm_timer_thread_ = std::thread(
            [this, interval_sec]() { PrewarmTimerWorker(interval_sec); });
        LOG(INFO) << "Started prewarm timer thread with interval: "
                  << interval_sec << " seconds";
    }

    LOG(INFO) << "Successfully started prewarm operation for shard "
              << shard_id;
    return true;
}

void DataStoreService::StopPrewarmOperation(uint32_t shard_id)
{
    // Check if current node is a standby node for this shard
    std::vector<uint32_t> shards_for_this_node =
        cluster_manager_.GetShardsForThisNode();
    ...
    {
        LOG(INFO) << "Current node is not a standby node for shard " << shard_id
                  << ", skip stopping prewarm operation";
        return;
    }
    ...
    {
        prewarm_data_store_->StopStandbySyncFileCache();
        LOG(INFO) << "Stopped standby sync file cache for shard " << shard_id;
    }

    // Then, stop timer worker thread
    if (prewarm_timer_thread_.joinable())
    {
        ...
        LOG(INFO) << "Stopped prewarm timer thread for shard " << shard_id;
    }

    // Finally, destroy prewarm data store
    DestroyPrewarmDataStore();

    LOG(INFO) << "Stopped prewarm operation for shard " << shard_id;
}

void DataStoreService::PrewarmTimerWorker(uint32_t interval_sec)
{
    const auto interval = std::chrono::seconds(interval_sec);
    LOG(INFO) << "Prewarm timer worker started with interval: " << interval_sec
              << " seconds";
    while (!prewarm_timer_stop_)
    {
        // Sleep with periodic checks for stop flag (allows faster shutdown)
        auto sleep_start = std::chrono::steady_clock::now();
        while (!prewarm_timer_stop_ &&
               (std::chrono::steady_clock::now() - sleep_start) < interval)
        {
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
        }

        // Check stop flag after sleep
        if (prewarm_timer_stop_)
        {
            LOG(INFO) << "Prewarm timer worker thread exiting due to stop flag";
            break;
        }

        // Check if prewarm data store exists
        if (prewarm_data_store_ == nullptr)
        {
            LOG(INFO)
                << "Prewarm data store not found, skipping prewarm operation";
            continue;
        }

        // Call StandbySyncFileCache which will:
        // 1. Execute prewarm operation
        // 2. Wait for completion
        // 3. Clean up files
        LOG(INFO)
            << "Triggering periodic prewarm operation via StandbySyncFileCache";
        prewarm_data_store_->StandbySyncFileCache();
    }

    LOG(INFO) << "Prewarm timer worker thread exiting";
}

bool DataStoreService::CreatePrewarmDataStore(
    uint32_t shard_id, std::unordered_set<uint16_t> &&bucket_ids, int64_t term)
{
    if (prewarm_data_store_ != nullptr)
    {
        LOG(WARNING) << "Prewarm data store already exists";
        return true;
    }

    // Initialize prewarm filter with bucket_ids
    if (data_store_factory_ != nullptr)
    {
        data_store_factory_->InitializePartitionFilter(shard_id,
                                                       std::move(bucket_ids));
    ...
    // Create ReadOnly EloqStoreDataStore instance
    // Use the same factory
    prewarm_data_store_ = data_store_factory_->CreateDataStore(
        false,  // create_db_if_missing
        shard_id,
        this,
        false,  // start_db - we'll start it manually
        term);
    ...
    if (!prewarm_data_store_->Initialize())
    {
        LOG(ERROR) << "Failed to initialize prewarm data store";
        prewarm_data_store_.reset();
        return false;
    }

    if (!prewarm_data_store_->StartDB(term))
    {
        LOG(ERROR) << "Failed to start prewarm data store";
        prewarm_data_store_.reset();
        return false;
    }

    // Switch to ReadOnly mode
    prewarm_data_store_->SwitchToReadOnly();

    LOG(INFO)
        << "Successfully created and started prewarm data store for shard "
        << shard_id;
    return true;
}

void DataStoreService::DestroyPrewarmDataStore()
{
    if (prewarm_data_store_ != nullptr)
    {
        prewarm_data_store_->Shutdown();
        prewarm_data_store_.reset();
        LOG(INFO) << "Destroyed prewarm data store";
    }
}
#endif
