...
Paged large objects (eloqkv `docs/08-paged-objects.md` §5, §9) break the one-`FlushRecord`-one-row assumption: a paged object's checkpoint unit is `PagedObjectFlush` (`tx_service/include/paged_tx_object.h`) — the metadata row plus every dirty page **by reference** plus every pending-delete range — carried as a single `FlushRecord` payload so the batching layer structurally cannot split it.

- **Page rows** live in the object table's keyspace under composite keys (`tx_service/include/page_key_codec.h`): a reserved 8-byte `\x00EKVPAGE` magic, the object key, a kind byte, and a big-endian page id. The composite `TxKey`'s `Hash()` returns the object key's hash, so pages co-locate with their object on every path.
- **The DSS client** (`store_handler/data_store_service_client.cpp`) expands one `PagedObjectFlush` into `1 + N` rows in its per-row arrays — null page buffer ⇒ `Delete`, non-null ⇒ `Put` — and cuts batches on record boundaries (the size check runs before a record is expanded), so one object is never split across two atomic batches. The metadata row carries the object's store-TTL attribute (deadline + slack); page rows never carry one.
- **Batch key order is established by the EloqStore adapter, not by the flush path.** A paged object's rows are deliberately not adjacent in the keyspace — page rows sit under the reserved `\x00EKVPAGE` prefix, the metadata row under the plain object key — so one object's emission is not in store order. `BatchWriteTask::SetBatch` asserts each batch is strictly increasing, and `EloqStoreDataStore::BatchWriteRecords` satisfies it by checking `is_sorted` and sorting (key/value/ts/op/ttl move together). Sorting cannot fix duplicates, so the emitted set is duplicate-free by construction: `FreePage` erases the frame, making the dirty and pending-delete sets disjoint, and live-id enumeration excludes both.
- **The post-flush callback** hands the flushed `commit_ts` to the payload (`OnPagedFlushApplied`): pages with `last_modified_ts_ <= commit_ts` become clean, pending-delete ranges with `freed_ts_ <= commit_ts` recycle to the free list. No watermark is stored anywhere; the guard's single input is the callback parameter.

**Store rows are validated before they are trusted (paged objects).** `PagedTxObject::InstallPage`/`InstallPageShared` call a type hook, `ValidatePageImage`, before handing bytes to the frame table, so a truncated, torn, or mis-served page row is a deterministic rejection rather than an out-of-bounds read inside the type's accessors. The generic layer checks what it owns — byte count against the object's page size, id liveness — and `PageFrameTable::DeserializeMeta` bounds every value it acts on: page size within `[kMinPageSize, kMaxPageSize]`, id high-water within `kMaxPageCount` (the free-list rebuild allocates and scans a bitmap of that size), pending ranges ordered and below the high-water. `RebuildFreeRanges` returns failure on an out-of-range live id instead of asserting. See eloqkv `docs/08-paged-objects.md` §5 for the full three-layer table, including the routing-identity check that gives a page an identity without carrying one.
