...
2. `ExecuteOn` returns an `ExecResult`: `Fail / Read / Write / Delete / Block / Unlock`. Only the (small) command result travels back, not the object. When a **remote** owner runs a TTL-mutating command it also returns its TTL facts in `ApplyResponse` (`ttl_reset`, `recover_cmd_image` snapshot, post-command `ttl`, `ttl_expired`; fields 10-13) so the coordinator — which never ran `ExecuteOn` — can still log the correct WAL record (`07-durability-and-recovery.md` §2.3, eloqdata/eloqkv#509).
