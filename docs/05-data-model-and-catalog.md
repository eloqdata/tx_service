...
- **Factory array indexing**: `GetCatalogFactory` computes `engine - 1` with no bounds check — passing `TableEngine::None` indexes `[-1]`. Also note two header quirks (verified, harmless but confusing): `cc_shard.h:297` declares the ctor parameter as `CatalogFactory *catalog_factory[6]` while the definition and member use 5; and `local_cc_shards.h:2089`'s in-class initializer lists hash/range factories in *swapped* slots — it is dead code, always overridden by the constructor's init list (`local_cc_shards.cpp:112`) which assigns range→slot 3 (`InternalRange`), hash→slot 4 (`InternalHash`).
