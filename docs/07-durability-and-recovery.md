...
4. Serialize one blob per ng (`node_txn_logs[ng_id]`), a concatenation of per-table sections:
   ...
   - **Object command section** (`:4767-4846`): same table header, then per object: key bytes, 8-byte `object_version`, 8-byte ttl, 4-byte commands length, 1-byte `ignore_previous_version` (overwrite flag), 2-byte command count, then length-prefixed serialized commands. For objects the WAL stores **commands, not resulting values**; replay re-applies them. For a key owned by a **remote** node group the coordinator never ran `ExecuteOn`, so the owner-side facts this section needs travel back in `ApplyResponse` (fields 10-13, eloqdata/eloqkv#509): the post-command object `ttl` written into the 8-byte ttl above (else the remote path logged `UINT64_MAX` and recovery could resurrect an expired object); `ttl_reset` + `recover_cmd_image`, a full-object snapshot the coordinator logs as an overwrite command via `CmdSetEntry::AddOverwriteCommandImage` because it holds no base object of its own; and `ttl_expired`, which makes the coordinator prepend a retire `DelCommand` overwrite record for the expired-then-recreated key. The owner reports the post-command ttl for the expired-recreation case too (`UINT64_MAX`, not the stale expired value — a fix that also cured a pre-existing *local* recovery loss); an old owner omitting fields 10-13 falls back to `UINT64_MAX`. Local writes are unchanged: the coordinator is the owner and uses its own `RecoverTTLObjectCommand()` snapshot.
