...
2. Otherwise (built with `WITH_LOG_SERVICE`) every distinct tx node hosts a log server on `local_port + 2`; the local `::txlog::LogServer` is constructed with `txlog_group_replica_num` (default 3, `log_init.cpp:11`) and the build-selected log-state backend (plain RocksDB or RocksDB-cloud) from the in-tree `eloq_log_service/` — see [10-log-service.md](10-log-service.md). `log_server_->Start()` runs it in-process (`log_init.cpp:623`).
...
4. Serialize one blob per ng (`node_txn_logs[ng_id]`), a concatenation of per-table sections:
   ...
   - **Object command section** (`:4767-4846`): same table header, then per object: key bytes, 8-byte `object_version`, 8-byte ttl, 4-byte commands length, 1-byte `ignore_previous_version` (overwrite flag), 2-byte command count, then length-prefixed serialized commands. For objects the WAL stores **commands, not resulting values**; replay re-applies them. For a key owned by a **remote** node group the coordinator never ran `ExecuteOn`, so the owner-side facts this section needs travel back in `ApplyResponse` (fields 10-13, eloqdata/eloqkv#509; round-tripped on the single-object `ApplyCc` path only — the multi-object post-process has no snapshot/backfill wiring): the post-command object `ttl` written into the 8-byte ttl above (else the remote path logged `UINT64_MAX` and recovery could resurrect an expired object); `ttl_reset` + `recover_cmd_image`, a full-object snapshot the coordinator logs as an overwrite command via `CmdSetEntry::AddOverwriteCommandImage` because it holds no base object of its own; and `ttl_expired`, which makes the coordinator prepend a retire `DelCommand` overwrite record for the expired-then-recreated key. The owner reports the post-command ttl for the expired-recreation case too (`UINT64_MAX`, not the stale expired value — a fix that also cured a pre-existing *local* recovery loss); an old owner omitting fields 10-13 falls back to `UINT64_MAX`. The snapshot has a single capture point for local and remote owners alike: the owner shard serializes `RecoverTTLObjectCommand()` into `ObjectCommandResult::recover_cmd_image_` at execution time, and the coordinator always logs that result-carried image (remote responses copy it onto the wire; the coordinator never rebuilds it from its own command).
...
2. `EloqLogAgent : TxLog` (`eloq_log_wrapper.h`) delegates to `txlog::LogAgent` (`tx_service/tx-log-protos/log_agent.h/.cpp`), the brpc client: per-log-group **leader cache** (`lg_leader_cache_`), channels to every log node (10 s timeout, 3 brpc retries, `log_agent.cpp:96-100`), async `LogService_Stub::WriteLog` (`log_agent.cpp:356-371`). A background `check_leader_thd_` continuously refreshes leaders via the braft route table (`LogAgent::CheckLeaderRun`, `log_agent.h:64`).
...
3. **Flush** — batches go through `AddFlushTaskEntry` → per-worker `pending_flush_work_` queues (with merge, `local_cc_shards.cpp:5820-5907`) consumed by `FlushDataWorker` threads. `FlushDataImpl` (`:5915-6100`) executes in order:
   ...
   For the progressive EloqStore path, `SyncPutAllData` reports cumulative serialized bytes after each partition's ckpt-ts fan-in. `FlushDataImpl` converts that watermark proportionally into the task's in-memory flush quota (with a 128-bit multiply and a final exact remainder release), so data-sync admission advances with durable partitions instead of waiting for the slowest partition. MVCC and persist-needing stores retain the full quota until their later durability boundary.
